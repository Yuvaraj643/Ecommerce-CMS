      ...
      <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th scope='col' class="px-6 py-4 font-medium text-gray-900">Toggle</th>
        ...
        <tbody class="divide-y divide-gray-100 border-t border-gray-100">
          {products.map((product, index) => (
            <tr class="hover:bg-gray-50" key={index}>
              ...
              <td class="px-3 py-4">
                <span
                  class={`inline-flex items-center gap-1 rounded-full ${product.enabled ? "bg-green-50" : "bg-red-50"} px-2 py-1 text-xs font-semibold ${product.enabled ? "text-green-600" : "text-red-600"} text-green-600`}
                >
                  <span class={`h-1.5 w-1.5 rounded-full ${product.enabled ? "bg-green-600" : "bg-red-600"}`}></span>
